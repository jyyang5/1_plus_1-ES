...
% NUM_OF_ITERATIONS:  number of maximum iterations
    ...
    while(t < NUM_OF_ITERATIONS && f(x_array(:,T))>10^(-8))%(norm(x_array(:,T)-OPTIMAL(:,1)) > TARGET_DISTANCE))
        ...
        % update GP iff. there is 40 candiate solutions
            ...
            theta = sigma*8*n;                 % length scale of GP
            ...
            fy_new = f(y_new);                                                     % evaluate the best offspring in that dim use true objective function
            xTrain(:, T) = y_new;                
            fTrain(T) = fy_new;

            if(fy_new >= f_x(T))                                                   % bad offspring
                if(para(1) >= 0.01)
                    sigma = sigma * 0.8;                                           % reduce step size
                else 
                    sigma = sigma * 0.9;  
                end
            else                                                                   % good offspring
    ...
    d1 = sprintf('point: ');
    d2 = sprintf("objective function value: %f", f_x(T-1));
    d3 = sprintf("mutation strength(last function evaluation): %f", sigma_funEva_array(T-1));
    %d4 = sprintf("mutation strength(last iteration): %f", sigma_iterate_array(T-1));
